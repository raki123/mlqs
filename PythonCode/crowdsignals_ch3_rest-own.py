...
dataset = pd.read_csv(dataset_path + 'chapter3_result_outliers-own.csv', index_col=0)
...
for col in [c for c in dataset.columns if not 'label' and not 'light_phone_lux' in c]:
...
original_dataset = pd.read_csv(dataset_path + 'chapter2_result-own.csv', index_col=0)
...
#new_dataset = LowPass.low_pass_filter(copy.deepcopy(dataset), 'light_phone_lux', fs, cutoff, order=10)
...
#DataViz.plot_dataset(new_dataset.ix[int(0.4*len(new_dataset.index)):int(0.43*len(new_dataset.index)), :], ['light_phone_lux', 'light_phone_lux_lowpass'], ['exact','exact'], ['line', 'line'])
...
DataViz.plot_dataset(dataset, ['acc_', 'gyr_', 'light_phone_lux', 'mag_', 'pca_', 'label'], ['like', 'like', 'like', 'like', 'like', 'like', 'like','like', 'like'], ['line', 'line', 'line', 'line', 'line', 'line', 'line', 'points', 'points'])

# Store the outcome.

dataset.to_csv(dataset_path + 'chapter3_result_final-own.csv')
